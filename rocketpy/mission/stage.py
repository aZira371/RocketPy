...
class Stage(AttachedItem):
    ...
    def __init__(
        ...
        if ignition_event is not None and ignition_event not in all_events:
            all_events.insert(0, ignition_event)
        if separation_event is not None and separation_event not in all_events:
            all_events.insert(0, separation_event)
    ...
    def validate(self):
        """Validate that this stage is correctly configured.

        Raises
        ------
        ValueError
            If *body* or *attachment* are ``None``.
        """
        if self.body is None:
            raise ValueError(f"Stage '{self.name}': body must not be None.")
        if self.attachment is None:
            raise ValueError(f"Stage '{self.name}': attachment must not be None.")
