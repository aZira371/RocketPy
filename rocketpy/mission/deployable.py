...
class Deployable(AttachedItem):
    ...
    def __init__(
        ...
        # Merge deployment_event into the events list so the base class
        # holds *all* events for this item.
        all_events = list(events or [])
        if deployment_event not in all_events:
            all_events.insert(0, deployment_event)
    ...
    def validate(self):
        ...
        ------
        ValueError
            If *body*, *attachment*, or *deployment_event* are ``None``.
        """
        if self.body is None:
            raise ValueError(f"Deployable '{self.name}': body must not be None.")
        if self.attachment is None:
            raise ValueError(f"Deployable '{self.name}': attachment must not be None.")
        if self.deployment_event is None:
            raise ValueError(
                f"Deployable '{self.name}': deployment_event must not be None."
            )
